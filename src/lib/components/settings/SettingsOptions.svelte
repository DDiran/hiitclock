...
{#if !$showSettings}
  <div class="flex flex-col gap-9 mb-12">
    <h1
      in:fly|local={{ y: 100 }}
      class="text-4xl text-center leading-9 font-bold font-serif">
      Welcome to Just HIIT
    </h1>
    <h2
      in:fly|local={{ y: 100, delay: 200 }}
      class="text-xl text-center leading-7 opacity-70 font-sans">
      Choose the best option to start configuring your perfect workout.
    </h2>
  </div>
  <form>
    <div
      in:fly|local={{ y: 100, delay: 400 }}
